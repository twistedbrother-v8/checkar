...
const contentFR = {
  ...
  cgu: [
    ...
    { titre: "3. Accès au service", texte: "L'application est accessible à toute personne disposant d'un accès internet et d'un compte utilisateur. Une version gratuite avec fonctionnalités limitées est disponible. Des fonctionnalités avancées sont accessibles via un abonnement Premium ou Ultra Premium. La création d'un compte nécessite une adresse email valide et un mot de passe. L'utilisateur est seul responsable de la confidentialité de ses identifiants." },
    ...
    { titre: "5. Responsabilité", texte: "Les informations fournies par l'application (procédures d'urgence, guide constat, état du véhicule, données de diagnostic OBD-2, codes défauts) sont données à titre indicatif et ne remplacent en aucun cas un diagnostic professionnel réalisé par un technicien qualifié. L'effacement de codes défauts via l'application ne résout pas la panne sous-jacente — l'utilisateur agit sous sa propre responsabilité. CheckApp Studio ne saurait être tenu responsable des décisions prises par l'utilisateur sur la base de ces informations. En cas d'urgence, contactez toujours les services officiels (15, 17, 18, 112)." },
...
const contentEN = {
  ...
  cgu: [
    ...
    { titre: "3. Access to the service", texte: "The application is accessible to anyone with internet access and a user account. A free version with limited features is available. Advanced features are accessible via a Premium or Ultra Premium subscription. Creating an account requires a valid email address and password. The user is solely responsible for the confidentiality of their credentials." },
    ...
    { titre: "5. Liability", texte: "Information provided by the application (emergency procedures, accident report guide, vehicle status, OBD-2 diagnostic data, fault codes) is for informational purposes only and does not replace a professional diagnosis by a qualified technician. Clearing fault codes via the application does not resolve the underlying issue — the user acts under their own responsibility. CheckApp Studio cannot be held liable for decisions made by users based on this information. In an emergency, always contact official services (15, 17, 18, 112)." },
